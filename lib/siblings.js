...
const SIBLINGS = {
  ...
  maat:     'the dashboard',
  peitho:   'WRITES the go-to-market copy (ZOILUS judges it, and never its own)',
  pyrrho:   'owns what the NUMBERS say (ZOILUS owns whether the craft holds)',
};
...
const LENS_ROUTE = {
  ...
  completion: 'horkos',
  // PEITHO writes copy; it may never grade its own pitch, so ZOILUS keeps the
  // copy verdict and routes only the market-positioning judgment to it.
  positioning: 'peitho',
  // a statistical claim is PYRRHO's jurisdiction: ZOILUS can see that an
  // analysis is well-crafted and still not know whether the data earns it.
  statistics: 'pyrrho',
  'claim-vs-evidence': 'pyrrho',
};
