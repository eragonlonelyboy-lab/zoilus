...
function parse(md) {
  ...
  for (const b of lensBlocks) {
    ...
    for (const fl of b.split('\n').filter((x) => /^-\s/.test(x))) {
      ...
      // Separator is a pipe, not a dash or a comma: a summary contains commas, and a
      // dash gets rewritten by an em-dash sweep, silently desyncing serialize from parse.
      const fm2 = fl.match(/^-\s*\[(\w+)\]\s*(.*?)(?:\s*\|\s*fix:\s*(.*))?$/);
